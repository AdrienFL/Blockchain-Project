...
public class Process extends UntypedAbstractActor {
    ...
    private void abortReceived(int newBallot, ActorRef pj) {
        this.mayCrash();
        if (!this.silentMode) {
            // log.info("abort received from " + pj.path().name() + " with ballot " +
            // newBallot);
    ...
    private void readReceived(int newBallot, ActorRef pj) {
        this.mayCrash();
        if (!this.silentMode) {
            // log.info("read received from " + pj.path().name() + " with ballot " +
            // newBallot);
    ...
    private void gatherReceived(int newBallot, int estballot, int est, ActorRef pj) {
        this.mayCrash();
        if (!this.silentMode) {
            // log.info("gather received from " + getSender().path().name() + " with ballot
            // " + newBallot);
            if (!this.abortList.contains(newBallot)) {
                this.states.put(pj, new Pair(est, estballot));
                if (this.states.size() > N / 2) {
                    getContext().system().scheduler().scheduleOnce(Duration.ofMillis(0), getSelf(), "majority",
    ...
    private void ackReceived(int newBallot, ActorRef pj) {
        this.mayCrash();
        if (!this.silentMode) {
            // log.info("ack received from " + pj.path().name() + " with ballot " +
            // newBallot);
            if (!this.abortList.contains(newBallot)) {
                ...
                if (this.ackMajorityMap.get(newBallot) > N / 2) {
                    getContext().system().scheduler().scheduleOnce(Duration.ofMillis(0), getSelf(), "ackmajority",
                            getContext().system().dispatcher(), ActorRef.noSender());
                }
    ...
    private void majorityReceived() {
        if (!silentMode) {
            ...
            // log.info("max ballot " + maxEstBallot + " from process " +
            // maxStateProcess.path().name());
            if (maxEstBallot > 0) {
    ...
    private void propose(int v) {
        if (!this.silentMode) {
            ...
            getContext().system().scheduler().scheduleOnce(Duration.ofMillis(random.nextInt(10)), self(), "propose",
                    getContext().system().dispatcher(), ActorRef.noSender());
        }
    }
    ...
    public void onReceive(Object message) throws Throwable {
        if (message instanceof Members) { // save the system's info
            ...
            if (!this.silentMode) {
                ...
                // log.info("impose received from " + getSender().path().name() + " with ballot
                // " + m.ballot + " and proposal " + m.proposal +" (imposeballot " +
                // this.imposeballot+")" + " (readballot " + this.readballot + ")");
            ...
            if (m.equals("hold")) {
                // log.info("p" + self().path().name() + " hold");
                this.proposemode = false;
            }

            if (m.equals("propose")) {
                if (this.proposemode && !this.silentMode) {
    ...
    private void decideReceived(int newProposal) {
        this.mayCrash();
        if (!silentMode) {
            // log.info("decide received with proposal "+ newProposal + " from " +
            // getSender().path().name());
